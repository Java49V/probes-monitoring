...
//	    SensorRange range = mapRanges.get(sensorId);
//
//	    if (range != null && range.minValue() == 0.0 && range.maxValue() == 0.0) {
//	        return null;
//	    }
//
//	    return range == null ? getRangeFromService(sensorId) : range;
...
//			getRangeFromService(id);
	...
	private SensorRange getRangeFromService(long id) {
		...
		try {
			ResponseEntity<SensorRange> responseEntity = restTemplate.exchange(getFullUrl(id), HttpMethod.GET, null,
					SensorRange.class);
	...
	private SensorRange getDefaultRange() {
		...
		return new SensorRange(providerConfiguration.getMinDefaultValue(), providerConfiguration.getMinDefaultValue());
	}

	private String getFullUrl(long id) {
		String res = String.format("http://%s:%d%s/%d", providerConfiguration.getHost(),
				providerConfiguration.getPort(), providerConfiguration.getUrl(), id);
		log.debug("url:{}", res);
		return res;
	}
